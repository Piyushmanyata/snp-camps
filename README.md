...
1. **Self-register** with **Aadhaar only** → verify → account created → **auto sign-in**
...
```
# Aadhaar eKYC / OTP provider (until set, Verhoeff checksum verifies)
# AADHAAR_VERIFY_URL=
# AADHAAR_LOOKUP_URL=
# SMS_WEBHOOK_URL=
...
1. **Verify**: `AADHAAR_VERIFY_URL` — POST `{ "aadhaar": "12digits", "action": "verify" }`. Until set, checksum-only verification is used for self-reg.  
