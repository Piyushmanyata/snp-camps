/**
 * Empirical stress tests for the mark_seen RPC (D22/D25).
 *
 * Split out of empirical-challenge.test.mjs: these need a live Postgres, and a
 * DB test that skips is a failure, not a pass (AGENTS.md). Only the .db.test.mjs
 * suffix puts them under the zero-skip guard in scripts/run-db-tests.mjs.
 */
  ...
  try {
    ...
    await client.query(
      `insert into public.camp_days (id, camp_id, day_date, seat_limit, printing_open)
       values ($1, $2, (timezone('Asia/Kolkata', now()))::date, 100, true)`,
      [dayId, campId],
    );
    await client.query("commit");
    return { campId, dayId };
