...
test("readiness turns red when one required RPC is missing", async (t) => {
  ...
  try {
    await admin.query("drop function public.mark_patient_printed(uuid, integer)");
    const facts = await probeFacts();
    assert.equal(facts.functions.mark_patient_printed, false);
...
test("readiness rejects a wrong-overload substitute", async (t) => {
  ...
  try {
    await admin.query("drop function public.mark_patient_printed(uuid, integer)");
    await admin.query(`
      create function public.mark_patient_printed(uuid, text)
      returns integer
      language sql
      immutable
      as $$ select 0 $$
...
test("readiness detects a revoked authenticated RPC grant", async (t) => {
  ...
  try {
    await admin.query(
      "revoke execute on function public.mark_patient_printed(uuid, integer) from authenticated",
    );
    const facts = await probeFacts();
    assert.equal(facts.functions.mark_patient_printed, true);
    assert.equal(facts.grants.mark_patient_printed_authenticated_execute, false);
