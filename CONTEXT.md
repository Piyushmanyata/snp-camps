...
* **Status token / status link**: Un-guessable token on the patient row; open `/s/<token>` with no sign-in to see camp day, venue, and `registered` or `seen`. No position. Used in registration SMS. Not the staff-scan QR.
...
* **Walk-in vs pre-reg**: Registering on an **active Camp Day (today, Asia/Kolkata)** and printing is one desk visit: saved, then present, still `registered`. Registering for a future day stays `registered` and prints nothing. **No desk mode toggle** — the system uses the camp day date.
* **Volunteer Desk**: The one station staff operate (`/volunteer`; admins get the same surface on `/admin`). Two big buttons — **Print prescription** and **Mark seen** — each opens scan-or-type. No Live Queue panel. Mobile-first, because phones do the scanning and a laptop does the printing. The **only** place walk-ins are registered (online self-registration covers pre-registration only).
* **Self-registration**: Patient self-service registration online by scanning the QR on their Aadhaar card (`/self-register`). Status is always `registered`. Requires no SMS, no OTP, and no eKYC provider configuration.
* **Aadhaar scanned**: Details were read from the QR on a physical Aadhaar card and are assumed authentic — **no** signature check and **no** OTP is performed, so this asserts provenance of the *data*, not confirmation of *identity*. The decoder reports a card or nothing; the registration form decides completeness. Desk may fill a partial and keep scanning until the Aadhaar lock fields are all present. Self-registration requires a complete card before any field is filled. Absence of a scan is normal for walk-ins typed in at the desk and indicates self-declared details.
...
* **Patients and field staff read Hinglish** — the self-registration flow, the status page, SMS, the public home page, and every field desk surface: register, volunteer desk, clinical desk, scanner, print actions.
...
* **Status Token Boundary**: Passwordless `/s/<token>` provides public status tracking via the `patient_status_by_token` RPC, which is **service-role only** and returns camp day, venue, and `registered` / `seen` with PII, phone, address, Aadhaar details, and any position number stripped.
