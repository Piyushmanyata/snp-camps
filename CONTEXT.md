...
* **Print window**: The admin-controlled gate that lets Print prescription and Mark seen appear on the Volunteer Desk. Open when an admin has opened printing for that camp day (`printing_open = true`). Every new Camp Day starts closed. Per day, not per camp. Admin opens it from **Admin → Camp days & print window** with **Enable prescription printing** on that day's row. The database refuses presence writes while it is closed ([ADR 0020](docs/adr/0020-print-window.md), [ADR 0027](docs/adr/0027-admin-can-enable-printing-on-any-camp-day.md)).
...
* **Walk-in vs pre-reg**: Registering on an active Camp Day with printing open is one desk visit: saved, then present, still `registered`. Registering for a day where printing is closed stays `registered` and prints nothing. Printing requires the **Print window** to be open for that Camp Day. **No desk mode toggle** — the per-day admin switch decides whether Print appears.
