...
* **Print window**: The admin-controlled gate that lets Print prescription and Mark seen appear on the Volunteer Desk. Open only when the Camp Day's date is today (`Asia/Kolkata`) **and** an admin has opened printing for that day. Every new Camp Day starts closed. Per day, not per camp. The database refuses presence writes while it is closed ([ADR 0020](docs/adr/0020-print-window.md)).
