...
* **Name-search**: For patients who lost their paper, forget the number, and gave no phone. Prefix match on normalised name (case-folded, whitespace-collapsed), active camp, **all queue statuses** (`registered` / `waiting` / `seen`) so desk recovery is not limited to pre-reg only; rows show name, age, address (locality), and status.
