...
test("public entry points and protected-route redirects", async ({ page, request }) => {
  const health = await request.get("/api/health");
  expect(health.ok()).toBeTruthy();

  await page.goto("/");
  await expect(page.getByRole("heading", { name: "Medical Camp Desk" })).toBeVisible();
...
test("credential forms never put secrets in the URL before hydration", async ({ browser }) => {
...
test("admin can sign in, inspect the patient desk, and sign out", async ({ page }) => {
  await loginStaff(page, "admin");
  await expect(page.getByRole("heading", { name: "Admin" })).toBeVisible();
  await expect(page.getByText("Camps & camp days", { exact: true })).toBeVisible();

  await page.goto("/admin/patients");
  await expect(page.getByRole("heading", { name: "Patient desk" })).toBeVisible();
  await page.getByLabel("Filter list").fill(env("E2E_PATIENT_REG_NO"));
  await expect(page.getByText(`#${env("E2E_PATIENT_REG_NO")}`, { exact: true })).toBeVisible();
...
test("volunteer can sign in and safely review a patient", async ({ page }) => {
  ...
  await expect(review.getByRole("button", { name: "Assign doctor · mark seen" })).toBeDisabled();

  await page.getByRole("button", { name: "Sign out" }).click();
  await expect(page).toHaveURL(/\/$/);
});

test("doctor can sign in and review without mutating patient status", async ({ page }) => {
  ...
  await expect(review).toBeVisible();
  await expect(review.getByRole("button", { name: "Confirm patient · mark seen" })).toBeVisible();
...
test("patient can sign in with registration number and sign out", async ({ page }) => {
  ...
  await expect(page.getByText(env("E2E_PATIENT_NAME"), { exact: true })).toBeVisible();
  await expect(page.getByText(`#${env("E2E_PATIENT_REG_NO")}`, { exact: true })).toBeVisible();
...
test("patient phone OTP uses the real configured SMS provider", async ({ page }) => {
  test.skip(
