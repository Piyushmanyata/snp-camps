-- #70 — Atomic FCFS queue position for the public status page.
-- Canonical order EVERYWHERE for waiting peers:
--   queued_at ASC NULLS LAST, reg_no ASC, id ASC
-- Least-privilege projection only; EXECUTE granted to service_role/postgres
-- (status page uses createServiceRoleClient). No patient-table SELECT for anon.
...
BEGIN
  ...
  IF v_status = 'waiting'::public.queue_status THEN
    -- 1-based rank among camp waiting peers; same snapshot/order as FCFS queue.
...
COMMENT ON FUNCTION public.patient_status_by_token(text) IS
  'Public status-page projection (#70). Lookup by status_token; waiting patients get 1-based FCFS queue_position among camp peers ordered by (queued_at ASC NULLS LAST, reg_no ASC, id ASC). Non-waiting → queue_position NULL. Zero rows for unknown/malformed tokens. EXECUTE: service_role/postgres only.';
