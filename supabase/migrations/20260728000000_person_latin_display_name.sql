-- #112 — Latin display name for non-Latin scanned Aadhaar registrations
-- Adds display_name to persons and patients tables, updates register_patient_idempotent.

DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'persons' AND column_name = 'display_name'
  ) THEN
    ALTER TABLE public.persons ADD COLUMN display_name text;
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'patients' AND column_name = 'display_name'
  ) THEN
    ALTER TABLE public.patients ADD COLUMN display_name text;
  END IF;
END $$;

DROP FUNCTION IF EXISTS public.register_patient_idempotent(uuid,uuid,text,text,integer,text,text,text,text,uuid,uuid,uuid,boolean,boolean);
DROP FUNCTION IF EXISTS public.register_patient_idempotent(uuid,uuid,text,text,integer,text,text,text,text,uuid,uuid,uuid,boolean,boolean,boolean,text,timestamptz,text);
DROP FUNCTION IF EXISTS public.register_patient_idempotent(uuid,uuid,text,text,integer,text,text,text,text,uuid,uuid,uuid,boolean,boolean,boolean,text,timestamptz,text,text);
DROP FUNCTION IF EXISTS public.register_patient_idempotent(uuid,uuid,text,text,integer,text,text,text,text,uuid,uuid,uuid,boolean,boolean,boolean,text,timestamptz,text,text,date);
DROP FUNCTION IF EXISTS public.register_patient_idempotent(uuid,uuid,text,text,integer,text,text,text,text,uuid,uuid,uuid,boolean,boolean,boolean,text,timestamptz,text,text,date,text);
...
declare
  ...
  v_provenance text := coalesce(p_provenance, 'self_declared');
  v_is_card boolean := v_provenance in ('card_verified', 'ekyc_verified') or p_duplicate_key is not null;
begin
  ...
  select p.id, p.reg_no, coalesce(p.display_name, p.full_name), p.camp_day_id, d.day_date, p.queue_status
  ...
  if p_duplicate_key is not null and length(trim(p_duplicate_key)) > 0 then
    ...
    if v_person_id is not null then
      ...
      -- New Person: create in persons table with locks set for card verification
      ...
      insert into public.persons (
        ...
        v_aadhaar,
        p_duplicate_key,
        case when v_is_card then now() else null end,
        case when v_is_card then now() else null end,
        v_created_by,
        now()
      )
...
CREATE OR REPLACE FUNCTION public.register_patient_idempotent(
  ...
  SELECT * FROM public.register_patient_idempotent(
    ...
    p_camp_day_id, p_aadhaar_duplicate_override, p_likely_duplicate_override,
    false, NULL, NULL, NULL, 'self_declared', NULL, NULL, NULL
  );
$$;
...
CREATE OR REPLACE FUNCTION public.register_patient_idempotent(
  ...
  SELECT * FROM public.register_patient_idempotent(
    ...
    p_phone, p_email, p_aadhaar_last4, p_user_id, p_created_by,
    p_camp_day_id, p_aadhaar_duplicate_override, p_likely_duplicate_override,
    p_self_service, p_aadhaar_hash, p_aadhaar_verified_at, p_aadhaar_kyc_ref,
    'self_declared', NULL, NULL, NULL
  );
$$;
