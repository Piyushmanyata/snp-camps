...
-- Prevents mutation of locked identity fields (aadhaar_last4, full_name) on public.persons.

DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'persons' AND column_name = 'aadhaar_locked_at'
  ) THEN
    ALTER TABLE public.persons ADD COLUMN aadhaar_locked_at timestamp with time zone;
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'persons' AND column_name = 'name_locked_at'
  ) THEN
    ALTER TABLE public.persons ADD COLUMN name_locked_at timestamp with time zone;
  END IF;
END $$;

-- Trigger function enforcing lock immutability on persons table
...
AS $$
BEGIN
  IF OLD.aadhaar_locked_at IS NOT NULL AND NEW.aadhaar_last4 IS DISTINCT FROM OLD.aadhaar_last4 THEN
    RAISE EXCEPTION 'Aadhaar field is locked and cannot be modified';
  END IF;

  IF OLD.name_locked_at IS NOT NULL AND NEW.full_name IS DISTINCT FROM OLD.full_name THEN
...
DROP TRIGGER IF EXISTS enforce_person_field_locks_trg ON public.persons;
CREATE TRIGGER enforce_person_field_locks_trg
  BEFORE UPDATE ON public.persons
  FOR EACH ROW
  EXECUTE FUNCTION public.enforce_person_field_locks();

-- Update register_patient_idempotent to lock aadhaar and name on scanned card registration
