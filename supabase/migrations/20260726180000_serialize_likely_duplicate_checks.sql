-- #67 — Serialize concurrent likely-duplicate checks without blocking overrides.
-- Defect: only the request UUID was advisory-locked, so two desks with different
-- request IDs could both observe no soft match and insert (check-then-insert race).
-- Fix: before soft lookup, take camp-scoped transaction advisory locks on the
-- normalized soft-match keys (name+age and/or phone), in stable sorted order.
...
begin
  ...
  -- #67: camp-scoped soft-duplicate serialization (not the request id).
  -- Keys use normalized/redacted values only (no full Aadhaar). Hash collisions
  -- only add safe extra serialization, never correctness failure.
  ...
  -- Re-check committed state after locks (and after waiting for peer commits).
  select p.reg_no
  into v_soft_reg
  from public.patients p
...
COMMENT ON FUNCTION public.register_patient_idempotent(
  ...
  'Staff registration. Soft-duplicate keys use camp-scoped xact advisory locks (#67). p_user_id ignored (#59).';
