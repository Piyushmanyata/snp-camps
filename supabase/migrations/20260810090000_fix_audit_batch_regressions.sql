-- Fix regressions introduced by audit batch 1 and batch 2 migrations.
-- Append-only: redefines affected functions and restores broken contracts.

-- ---------------------------------------------------------------------------
-- F1 — register_patient_idempotent
-- Walk-ins on non-full days auto-queue to 'waiting'.
-- Walk-ins on FULL days (v_original_limit IS NOT NULL) stay in 'registered' status.
-- Returns queued_at and checked_in_by columns.
-- Passes v_provenance to inner function, includes strict variable for tests.
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  -- Only if the day WAS full (v_original_limit IS NOT NULL) do we force
  -- queue_status = 'registered' to prevent overbooking the waiting queue.
  IF NOT v_existing_request AND v_original_limit IS NOT NULL THEN
...
-- ---------------------------------------------------------------------------
-- F2 — Restore 'authenticated read permitted patients' policy name
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- F3 — staff_person_kpis
-- Single 4-arg contract; the retired p_since overloads are dropped.
-- Leaderboard allows volunteer callers and attributes KPI credit strictly to created_by.
-- ---------------------------------------------------------------------------
DROP FUNCTION IF EXISTS public.staff_person_kpis(uuid, text, uuid, text);
DROP FUNCTION IF EXISTS public.staff_person_kpis(uuid, text, uuid, text, text);
DROP FUNCTION IF EXISTS public.staff_person_kpis(uuid, text, uuid, timestamptz, text);
...
REVOKE ALL ON FUNCTION public.staff_person_kpis(uuid, text, uuid, text) FROM PUBLIC, anon;
...
BEGIN
  ...
  v_invariants := jsonb_build_object(
    ...
    'migration_head_current', public.latest_applied_migration() = '20260810090000',
  ...
  v_grants := jsonb_build_object(
    ...
    'check_in_patient_authenticated_execute', CASE WHEN to_regprocedure('public.check_in_patient(uuid,integer)') IS NOT NULL THEN has_function_privilege('authenticated','public.check_in_patient(uuid,integer)','EXECUTE') ELSE false END,
    ...
    'desk_waiting_queue_authenticated_execute', CASE WHEN to_regprocedure('public.desk_waiting_queue(uuid,integer)') IS NOT NULL THEN has_function_privilege('authenticated','public.desk_waiting_queue(uuid,integer)','EXECUTE') ELSE false END,
