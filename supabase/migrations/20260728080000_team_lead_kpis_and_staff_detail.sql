-- Update staff_person_kpis for team_lead role and team rollup
...
begin
  ...
  where p.id = v_caller_id
    and p.role in ('admin', 'volunteer', 'doctor', 'team_lead')
  ...
  elsif p_role in ('volunteer', 'team_lead') then
  ...
  if p_role = 'doctor' then
    ...
    with team_members as (
      ...
      union
      select tm.id from public.profiles tm where tm.team_lead_id = p_user_id and tm.disabled_at is null
    )
    select
      count(distinct p.id)::bigint,
      count(distinct p.id) filter (
        where (p.created_by in (select member_id from team_members) and p.created_at >= v_since)
           or (
             p.checked_in_by in (select member_id from team_members)
             and coalesce(p.queued_at, p.seen_at, p.created_at) >= v_since
           )
      )::bigint,
      count(distinct p.id) filter (where p.queue_status = 'waiting')::bigint,
      count(distinct p.id) filter (where p.queue_status = 'seen')::bigint,
      v_label
    from public.patients p
    where (p.created_by in (select member_id from team_members) or p.checked_in_by in (select member_id from team_members))
      and p.camp_id = p_camp_id;
  else
    -- p_role = 'volunteer'
    return query
