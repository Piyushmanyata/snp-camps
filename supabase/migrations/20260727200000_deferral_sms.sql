-- Ticket #100 — Deferral SMS notification ledger.
-- Adds spectacles_deferral and surgery_deferral enum values to sms_delivery_kind.
-- Automatically enqueues SMS delivery rows on spectacles or surgery deferrals when patient phone is set.

-- Extend sms_delivery_kind ENUM
...
-- Re-create resolve_treatment_order to enqueue deferral SMS
...
begin
  ...
  -- Enqueue deferral SMS if deferred and patient has phone
  if v_action = 'deferred' then
    select * into v_patient
    from public.patients
    where id = v_order.patient_id;

    if v_patient.phone is not null and length(trim(v_patient.phone)) >= 10 then
      v_phone10 := regexp_replace(v_patient.phone, '\D', '', 'g');
      if length(v_phone10) >= 10 then
        ...
        if v_order.kind = 'spectacles' then
          ...
          values (v_patient.id, 'spectacles_deferral', 'pending', right(v_phone10, 4)::char(4))
          on conflict (patient_id, kind) do nothing;
        elsif v_order.kind = 'ot' then
          insert into public.sms_deliveries (patient_id, kind, state, phone_last4)
          values (v_patient.id, 'surgery_deferral', 'pending', right(v_phone10, 4)::char(4))
          on conflict (patient_id, kind) do nothing;
        end if;
      end if;
    end if;
...
-- Re-create doctor_submit_prescription to enqueue deferral SMS for auto-deferred orders
...
-- Update readiness_catalog_probe to include spectacles_deferral and surgery_deferral in v_sms_kinds
...
DECLARE
  ...
  v_sms_kinds text[] := ARRAY[
    'registration', 'reminder', 'spectacles_deferral', 'surgery_deferral'
  ];
BEGIN
