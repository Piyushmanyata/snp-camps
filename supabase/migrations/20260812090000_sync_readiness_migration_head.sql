-- Sync the readiness migration-head probe with the repository head.
--
-- 20260810120000_fix_desk_waiting_queue_ambiguous_id.sql advanced the repo head
-- and src/lib/readiness-contract.ts EXPECTED_MIGRATION_HEAD, but did not bump
-- public.latest_applied_migration(). On any environment built from the
-- repository the probe kept returning '20260810090000' while the contract and
-- the supabase_migrations ledger both said '20260810120000', so the readiness
-- handler's applied_head_agreement check failed on every fresh deploy.
--
-- Forward-only fix: this migration is the new head and reports itself.
...
-- ---------------------------------------------------------------------------
-- Make the readiness probe self-maintaining.
--
-- readiness_catalog_probe() hard-coded the expected head as a literal, so it
-- had to be hand-edited on every migration. It was already stale: it compared
-- against 20260810090000 while the repo head was 20260810120000, and only
-- looked healthy because latest_applied_migration() was stale by the same
-- amount. Compare against the migration ledger instead, so the fact tracks the
-- database rather than a literal someone has to remember to bump.
-- Body is otherwise byte-identical to 20260810090000.
-- ---------------------------------------------------------------------------
